"use client";

import { useEffect, useState } from "react";
import { doc, onSnapshot } from "firebase/firestore";
import { getFirebaseDb } from "@/lib/firebase.client";
import s from "./mobileAppBanner.module.css";

const DISMISS_KEY = "nci_app_banner_dismissed";

type MobileAppSettings = {
  enabled?: boolean;
  appStoreUrl?: string;
  googlePlayUrl?: string;
};

/* ── SVG İkonlar ── */
function AppleIcon({ size = 20 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
      <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z" />
    </svg>
  );
}

function PlayStoreIcon({ size = 20 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
      <path d="M3.609 1.814L13.792 12 3.61 22.186a.996.996 0 01-.61-.92V2.734a1 1 0 01.609-.92zm10.89 10.893l2.302 2.302-10.937 6.333 8.635-8.635zm3.199-1.707l2.108 1.222a1 1 0 010 1.74l-2.108 1.222-2.536-2.536 2.536-2.648zM5.864 2.658L16.8 8.99l-2.302 2.302-8.635-8.635z" />
    </svg>
  );
}

/**
 * Mobil kullanıcılara App Store indirme çağrısı yapan yapışkan banner.
 * - Sadece mobilde görünür (CSS ile)
 * - X ile kapatılabilir, sessionStorage ile oturum boyunca gizli kalır
 * - Admin settings/public → mobileApp alanından URL ve aktiflik okur
 */
export default function MobileAppBanner() {
  const [settings, setSettings] = useState<MobileAppSettings | null>(null);
  const [dismissed, setDismissed] = useState(true);

  useEffect(() => {
    // Daha önce kapatıldı mı?
    try {
      if (sessionStorage.getItem(DISMISS_KEY) === "1") {
        setDismissed(true);
        return;
      }
    } catch {
      //
    }
    setDismissed(false);
  }, []);

  useEffect(() => {
    const db = getFirebaseDb();
    const ref = doc(db, "settings", "public");
    const unsub = onSnapshot(ref, (snap) => {
      if (snap.exists()) {
        const d = snap.data();
        const ma = d?.mobileApp && typeof d.mobileApp === "object" ? d.mobileApp : {};
        setSettings({
          enabled: Boolean(ma.enabled),
          appStoreUrl: String(ma.appStoreUrl || ""),
          googlePlayUrl: String(ma.googlePlayUrl || ""),
        });
      }
    });
    return () => unsub();
  }, []);

  const handleDismiss = () => {
    setDismissed(true);
    try {
      sessionStorage.setItem(DISMISS_KEY, "1");
    } catch {
      //
    }
  };

  // Gösterme koşulları
  if (!settings?.enabled) return null;
  if (!settings.appStoreUrl && !settings.googlePlayUrl) return null;

  const url = settings.appStoreUrl || settings.googlePlayUrl || "#";
  const isApple = Boolean(settings.appStoreUrl);

  return (
    <div className={s.banner} data-hidden={dismissed ? "true" : "false"}>
      <div className={s.inner}>
        <div className={s.appIcon}>
          {isApple ? <AppleIcon size={24} /> : <PlayStoreIcon size={24} />}
        </div>
        <div className={s.textWrap}>
          <div className={s.title}>6&apos;ncı e-ticaret</div>
          <div className={s.subtitle}>
            {isApple ? "App Store'da ücretsiz" : "Google Play'de ücretsiz"}
          </div>
        </div>
        <a
          href={url}
          target="_blank"
          rel="noopener noreferrer"
          className={s.badge}
        >
          {isApple ? <AppleIcon size={16} /> : <PlayStoreIcon size={16} />}
          İndir
        </a>
      </div>
      <button
        type="button"
        className={s.close}
        onClick={handleDismiss}
        aria-label="Kapat"
      >
        ✕
      </button>
    </div>
  );
}
